...
class ChartManager {
  ...
  constructor(
    ...
    this.sheetDataBase =
      SpreadsheetApp.getActiveSpreadsheet().getSheetByName(sheetNameDatabase);
  ...
  inputDatabase_() {
    ...
    for (let i = 0; i < results.length; i++) {
      ...
      switch (results[i].properties[this.itemName].type) {
        case "title":
          ...
          item =
            results[i].properties[this.itemName]?.title?.[0]?.text?.content;
          break;
        default:
      ...
      if (item === undefined) {
        console.log(
          `Item data for record ${i} is undefined: ${
            results[i].properties[this.itemName]?.title?.[0]?.text?.content
          }`
        );
        continue;
        ...
        console.log(
          `Category data for record ${i} is undefined: ${
            results[i].properties[this.categoryName]?.select?.name
          }`
        );
        continue;
      } else if (pay === undefined) {
        console.log(
          `Pay data for ${item} is undefined: ${
            results[i].properties[this.payName].number
          }`
        );
        continue;
      } else if (pay === null) {
        console.log(
          `Pay data for record ${item} is null: ${
            results[i].properties[this.payName].number
          }`
        );
      } else if (pay === undefined) {
        console.log(
          `Pay data for record ${item} is undefined: ${
            results[i].properties[this.payName].number
          }`
        );
      } else if (pay === 0) {
    ...
    const refData = this.sheetDataBase
    ...
    refData.forEach(([category, pay]) => {
      ...
      for (let i = 0; i < categoryArray.length; i++) {
        totals[categoryArray[i]] =
          (totals[categoryArray[i]] || 0) + Number(pay);
      }
    });
    ...
    switch (relationIds) {
      ...
      default:
        ...
        for (let relationId of relationIds) {
          ...
          let pageContent =
            pageInfo.properties[this.categoryName].title[0].plain_text.trim();
  ...
  createPieChart() {
    ...
    const sheetChart =
      SpreadsheetApp.getActiveSpreadsheet().getSheetByName(sheetNameChart);
    if (sheetChart.getCharts() === null) {
      ...
      if (checkTableExist === -1) {
        console.log(
          "There is no chart with the target chart name in the sheet. Will create a new chart."
        );
        // Builds and insert the chart
        chartBuilder = sheetChart
          .newChart()
        ...
        console.log(
          "There is a previous chart in the sheet. Will update the existing chart."
        );
...
function manageChart_1() {
  const chart_1 = new ChartManager(
    sheetName_1,
    tableId_1,
    item_1,
    category_1,
...
function manageChart_2() {
  const chart_2 = new ChartManager(
    sheetName_2,
    tableId_2,
    item_2,
    category_2,
...
function manageChart_3() {
  const chart_3 = new ChartManager(
    sheetName_3,
    tableId_3,
    item_3,
    category_3,
